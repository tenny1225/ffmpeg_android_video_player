...
Java_com_example_tenny_myapplication1_DecoderJniLib_stringFromJNI(
        JNIEnv *env,
        jobject /* this */) {
...
Java_com_example_tenny_myapplication1_DecoderJniLib_decode(JNIEnv *env, jclass type, jstring input_,
                                                          jstring output_) {
    //获取输入输出文件名
    ...
    while (av_read_frame(pFormatCtx, packet) >= 0) {
        ...
        if (packet->stream_index == v_stream_idx) {
            ...
            if (got_picture) {
               ...
               /* glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0,  pFrameYUV->width,
                                pFrameYUV->height, GL_RGB, GL_UNSIGNED_BYTE,
                                pFrameYUV->data[0]);*/
